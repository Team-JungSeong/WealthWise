...
const BudgetSimulationPage: React.FC = () => {
  ...
  const simulationResults = {
    ...
    savingsDeficit: Math.max(
      0,
      (parseInt(savingsGoal) || 0) - ((parseInt(income) || 0) - totalExpenses)
    ),
    recommendedBudget: expenseCategories.map((category) => ({
      category,
      current: parseInt(expenses[category.id]) || 0,
      recommended: Math.round(
        ((parseInt(income) || 0) - (parseInt(savingsGoal) || 0)) *
          (category.id === "housing"
            ? 0.3
            : category.id === "food"
            ? 0.2
  ...
  return (
    <Layout>
      <PageHeader>
        ...
        <PageSubtitle>
          월 수입과 지출을 분석하여 최적의 예산 계획을 수립하고, 재정 목표
          달성을 위한 저축 계획을 세워보세요.
        </PageSubtitle>
      </PageHeader>
            ...
            <ExpensesContainer>
              {expenseCategories.map((category) => (
                <ExpenseItem key={category.id}>
                  <FormLabel style={{ width: "120px", margin: 0 }}>
                    {category.name}
                  </FormLabel>
                  <Input
                    type="number"
          ...
          <FormContainer>
            ...
            <ResultsContainer>
              <Card variant="outlined">
                <h3>수입 및 지출 요약</h3>
                <ResultValue>
                  {formatCurrency(simulationResults.monthlyNetIncome)}
                </ResultValue>
                <p>월 수입</p>

                <ResultValue>
                  {formatCurrency(simulationResults.totalExpenses)}
                </ResultValue>
                <p>총 지출</p>

                <ResultValue>
                  {formatCurrency(simulationResults.remainingForSavings)}
                </ResultValue>
                <p>저축 가능액</p>
                ...
                {simulationResults.savingsDeficit > 0 && (
                  <>
                    <ResultValue style={{ color: "#F44336" }}>
                      {formatCurrency(simulationResults.savingsDeficit)}
                    </ResultValue>
                    <p>저축 목표 부족액</p>
                  </>
                )}
            ...
            <div>
              <h3>분석 결과</h3>
              <p>
                {simulationResults.remainingForSavings >=
                simulationResults.savingsGoal
                  ? `현재 지출 패턴으로는 월 저축 목표인 ${formatCurrency(
                      simulationResults.savingsGoal
                    )}을 달성할 수 있습니다. ` +
                    `예상 월 저축액은 ${formatCurrency(
                      simulationResults.remainingForSavings
                    )}입니다.`
                  : `현재 지출 패턴으로는 월 저축 목표인 ${formatCurrency(
                      simulationResults.savingsGoal
                    )}을 달성할 수 없습니다. ` +
                    `저축 목표를 달성하려면 지출을 ${formatCurrency(
                      simulationResults.savingsDeficit
                    )} 줄여야 합니다.`}
              </p>
              <p>
                위 추천 예산을 참고하여 각 카테고리별 지출을 조정해보세요. 특히
                초과된 항목에서 지출을 줄인다면 저축 목표에 더 가까워질 수
                있습니다.
              </p>
            </div>
