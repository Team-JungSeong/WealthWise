  ...
  comments: [
    {
      ...
      date: "2023-09-20",
      content:
        '저도 2년 전에 주식을 처음 시작했었는데요, 초보자라면 개별 주식보다 ETF로 시작하는 것을 추천드립니다. 특히 KODEX200 같은 인덱스 ETF가 안정적이에요. 책은 "주식 시장에서 살아남기"를 추천합니다.',
      likes: 5,
    },
    {
      id: "c2",
      ...
      date: "2023-09-21",
      content:
        "초보자가 가장 많이 하는 실수는 욕심을 내서 단기간에 큰 수익을 내려고 하는 것입니다. 장기적 관점에서 투자하시고, 분산 투자를 하세요. 그리고 투자 전에 꼭 비상금을 마련해두세요!",
      likes: 12,
    },
    {
      id: "c3",
      ...
      date: "2023-09-21",
      content:
        '주식 공부를 위해서는 회사의 재무제표를 읽는 법을 배우는 것이 좋습니다. "재무제표 모르면 주식투자 절대 하지마라"라는 책을 추천합니다. 또한 경제 뉴스를 매일 챙겨보세요.',
      likes: 8,
    },
  ],
  likes: 56,
...
const ForumPostDetailPage: React.FC = () => {
  ...
  const [isMark, setIsMark] = useState(postDetail.isMark);
  ...
  return (
    <Layout>
      <PageHeader>
        <Link to="/community">&lt; 모든 커뮤니티로 돌아가기</Link>
      </PageHeader>
      ...
      <PostContainer>
        ...
        <PostMeta>
          ...
          <PostStatistics.Container>
            ...
            <PostStatistics.Category>
              {postDetail.categoryName}
            </PostStatistics.Category>
            <PostStatistics.Views>
        ...
        <PostActions>
          ...
          <ActionButton variant="tertiary">공유하기</ActionButton>
        </PostActions>
        <CommentsSection>
          ...
          <CommentsList>
            {postDetail.comments.map((comment) => (
              <CommentItem key={comment.id}>
                ...
                <CommentActions>
                  <ActionButton variant="primary">좋아요</ActionButton>
                  <ActionButton variant="secondary">댓글</ActionButton>
                </CommentActions>
              </CommentItem>
            ))}
          </CommentsList>
          <CommentForm>
            <TextArea
              ...
              style={{ resize: "none" }}
            />
            <ActionButton variant="primary">작성</ActionButton>
          </CommentForm>
        </CommentsSection>
        <RelatedTopics>
          <h3>관련 토픽</h3>
      ...
      <Sidebar>
        <WidgetTitle>커뮤니티 가이드라인</WidgetTitle>
        <WidgetContent>
          <p>서로를 존중하며 건설적인 대화를 나눠주세요.</p>
          <p>
            재정 관련 조언은 참고용으로만 활용하시고, 중요한 결정은 전문가와
            상담하세요.
          </p>
          <WidgetButton variant="light">전체 가이드라인 보기</WidgetButton>
        </WidgetContent>
      </Sidebar>
    </Layout>
  );
